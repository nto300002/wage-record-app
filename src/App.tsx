...
function App() {
  ...
  return (
    <main className="app-shell">
      <header className="app-header"><div><p className="brand-mark">🌱</p><h1>{easyDisplay ? 'こうちんシミュレーター' : '工賃シミュレーター'}</h1><p>{easyDisplay ? 'きょうの がんばりを かたちに' : '今日のがんばりを かたちに'}</p></div><div className="header-actions"><label className="easy-toggle"><span>{easyDisplay ? 'かんじなし' : '漢字あり'}</span><input aria-label="やさしい表示" type="checkbox" checked={easyDisplay} onChange={(event) => setEasyDisplay(event.target.checked)} /><span className="switch-track" aria-hidden="true"><span className="switch-thumb" /></span></label><button className="help-button" type="button">？ つかいかた</button></div></header>
