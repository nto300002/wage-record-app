  ...
  return (
    <main className="app-shell">
      ...
      <section className="card piecework-card" aria-labelledby="piecework-heading"><div className="section-heading"><span className="section-number">3</span><h2 id="piecework-heading">{easyDisplay ? 'できだか' : '出来高'}</h2><span>{easyDisplay ? 'つくった かずに おうじてもらえる きんがくです。' : '作った数におうじてもらえる金額です。'}</span></div>
        {pieceworks.map((piecework, index) => <div className="piecework-row" key={index}><strong>{easyDisplay ? 'できだか' : '出来高'} {index + 1}</strong><label>{easyDisplay ? 'なまえ' : 'なまえ'}<input aria-label={`${easyDisplay ? 'できだか' : '出来高'}${index + 1}の名前`} placeholder={easyDisplay ? 'しごとの なまえ' : 'しごとの名前'} /></label><label>{easyDisplay ? '1こ あたりの きんがく' : '1つあたりの金額'}<input aria-label={`${easyDisplay ? 'できだか' : '出来高'}${index + 1}の単価`} type="number" min="0" value={piecework.unitPrice || ''} onChange={(event) => updatePiecework(index, { unitPrice: Number(event.target.value) })} /> 円</label><label>{easyDisplay ? 'すうりょう' : '数量'}<input aria-label={`${easyDisplay ? 'できだか' : '出来高'}${index + 1}の数量`} type="number" min="0" value={piecework.quantity || ''} onChange={(event) => updatePiecework(index, { quantity: Number(event.target.value) })} /></label><output>{easyDisplay ? 'しょうけい' : '小計'}<strong>{piecework.unitPrice * piecework.quantity}円</strong></output><button type="button" aria-label={`${easyDisplay ? 'できだか' : '出来高'}${index + 1}を削除`} onClick={() => setPieceworks((current) => current.filter((_, itemIndex) => itemIndex !== index))}>🗑 {easyDisplay ? 'けす' : '削除'}</button></div>)}
...
function SessionCard({ title, id, session, workName, easyDisplay, onWorkChange, onChange, result }: { title: string; id: SessionKey; session: WageSession; workName: string; easyDisplay: boolean; onWorkChange: (value: string) => void; onChange: (patch: Partial<WageSession>) => void; result: number }) {
  return <section className="card session-card" aria-labelledby={`${id}-heading`}><div className="section-heading"><span className="section-number">{id === 'am' ? '1' : '2'}</span><h2 id={`${id}-heading`}>{title}</h2></div><div className="session-grid"><label>{easyDisplay ? 'しごとを えらぶ' : 'しごとを えらぶ'}<select aria-label={`${title}のしごと`} value={workName} onChange={(event) => onWorkChange(event.target.value)}><option value="">作業なし</option>{WORK_ITEMS.map((workItem) => <option key={workItem.id} value={workItem.id}>{easyDisplay ? workItem.easyName : workItem.name}</option>)}</select><span className="rate">{easyDisplay ? '1じかん あたり' : '1時間あたり'} {session.hourlyRate}円</span></label><div><span className="field-label">{easyDisplay ? 'はたらいた じかん' : 'はたらいた時間'}</span><div className="time-fields"><label><input aria-label={`${title}の時間`} type="number" min="0" value={session.hours || ''} onChange={(event) => onChange({ hours: Number(event.target.value) })} /> {easyDisplay ? 'じかん' : '時間'}</label><label><input aria-label={`${title}の分`} type="number" min="0" max="59" value={session.minutes || ''} onChange={(event) => onChange({ minutes: Number(event.target.value) })} /> {easyDisplay ? 'ふん' : '分'}</label></div></div><label>{easyDisplay ? 'かてん' : '能力の加点'}<select aria-label={`${title}の能力加点`} value={session.bonus} onChange={(event) => onChange({ bonus: Number(event.target.value) })}>{BONUSES.map((bonus) => <option key={bonus} value={bonus}>{bonus === 0 ? 'なし' : `${bonus}円`}</option>)}</select></label><output className="session-total">{title}{easyDisplay ? 'の こうちん' : 'の工賃'}<strong>{result}円</strong></output></div></section>
}
