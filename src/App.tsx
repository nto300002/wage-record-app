...
function App() {
  ...
  return (
    <main className="app-shell">
      ...
      {nameHelp && <div className="tour-backdrop name-help-backdrop" role="presentation" onClick={() => nameHelp.step === 0 ? setNameHelp((current) => current ? { ...current, step: 1 } : current) : nameHelp.step === 2 ? setNameHelp(null) : undefined}><p className={nameHelpBubble.below ? 'tour-bubble below' : 'tour-bubble'} style={{ top: nameHelpBubble.top, left: nameHelpBubble.left }}>{nameHelp.step === 0 ? (easyDisplay ? 'ここに きょう おこなった しごとが ないとき' : 'ここに今日おこなった仕事がないとき') : nameHelp.step === 1 ? (easyDisplay ? 'ここを くりっく' : 'ここをクリック') : (easyDisplay ? 'ここに しごとの なまえを にゅうりょく' : 'ここに仕事の名前を入力')}</p><button className="tour-close" type="button" onClick={(event) => { event.stopPropagation(); setNameHelp(null) }}>{easyDisplay ? 'とじる' : '閉じる'}</button></div>}
