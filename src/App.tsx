...
function PieceworkNameField({ index, piecework, easyDisplay, mode, onChange, onToggle }: { index: number; piecework: Piecework; easyDisplay: boolean; mode: 'select' | 'text'; onChange: (patch: Partial<Piecework>) => void; onToggle: () => void }) {
  const label = `${easyDisplay ? 'できだか' : '出来高'}${index + 1}の名前`
  return <label><span className="piecework-name-heading">{easyDisplay ? 'なまえ' : '名前'}<button type="button" aria-label="出来高の名前入力方式を切り替える" className="name-mode-button" onClick={onToggle}>{mode === 'select' ? (easyDisplay ? 'じゆうに いれる' : '自由入力にする') : (easyDisplay ? 'えらぶに もどす' : '選択に戻す')}</button></span>{mode === 'select' ? <select aria-label={label} value={piecework.name ?? ''} onChange={(event) => onChange({ name: event.target.value })}><option value="">{easyDisplay ? 'しごとを えらぶ' : '仕事を選ぶ'}</option>{WORK_ITEMS.map((workItem) => <option key={workItem.id} value={workItem.id}>{easyDisplay ? workItem.easyName : workItem.name}</option>)}</select> : <input aria-label={label} value={piecework.name ?? ''} placeholder={easyDisplay ? 'しごとの なまえ' : '仕事の名前'} onChange={(event) => onChange({ name: event.target.value })} />}</label>
}
